<template>
  <span class="flex justify-center w-full space-x-3 space-x-reverse">
    ...
    <button class="text-gray-900 hover:text-green-700" v-if="actions?.viewInvoice" @:click="showInvoice(data?.invoice)"><DocumentArrowDownIcon class="w-5 h-5" /></button>
...
const showInvoice = (invoice: String) => {
  $event('table-showinvoice-object', { invoice: invoice });
}
</script>

<template>
  <span class="flex justify-start w-full space-x-3 space-x-reverse">
    <NuxtLink v-if="actions?.view" :to="actions.view.path + data[property]"><EyeIcon class="w-5 h-5" /></NuxtLink>
    <NuxtLink
      class="text-gray-900"
      v-if="actions?.details"
      :to="actions.details.path ? actions.details.path + data[property] : ($route.path + '/' + data[property]).replace('//', '/')"
    >
...
defineProps({
  ...
  property: {
    type: String,
    default: 'id', // Default to 'id' but can be customized
  },
});

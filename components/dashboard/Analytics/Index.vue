<template>
    <div
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 py-6 px-6 sm:pt-0 w-full">
        <DashboardAnalyticsTable :head-items="{paymentMethod: 'وسيلة الدفع', orders: 'الطلبات'}" :body-items="[ {paymentMethod: 'VISA', orders: 10}, {paymentMethod: 'APPLEPAY', orders: 75} ]"/>
        <DashboardAnalyticsTable :head-items="{country: 'الدولة', orders: 'الطلبات', users: 'المستخدمين'}" :body-items="[ {country: 'الامارات العربية المتحدة', orders: 10, users: 40}, {country: 'السعودية', orders: 10, users: 40}, ]"/>
        <DashboardAnalyticsTable :head-items="{verifiedAuth: 'وسائل التسجيل المؤكدة', users: 'المستخدمين'}" :body-items="[ {verifiedAuth: ['الهاتف', 'sms'], users: 40}, {verifiedAuth: ['الهاتف', 'sms'], users: 40}, {verifiedAuth: ['الهاتف', 'sms'], users: 40}, ]"/>
        <DashboardAnalyticsTable :head-items="{payAverage: 'معدل شراء الطلبات', users: 'المستخدمين'}" :body-items="[ {payAverage: '3+', users: 40}, {payAverage: '3', users: 40}, {payAverage: '1', users: 7}, ]"/>
        <DashboardAnalyticsTable :head-items="{time: 'وقت الشراء', orders: 'الطلبات'}" :body-items="[ {time: '10:00 ص', orders: 10}, {time: '12:00 ص', orders: 10} ]"/>
        <DashboardAnalyticsTable :head-items="{buyer: 'المشتري', seller: 'معبر', orders: 'الطلبات'}" :body-items="[]"/>
      </div>
</template>

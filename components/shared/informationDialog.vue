                    ...
                    <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
                        ...
                        <DialogPanel
                            ...
                            <DialogTitle as="div" class="flex flex-col-reverse items-center">
                                ...
                                <h3 class="text-[16px] font-semibold leading-6 text-gray-900 py-1">نحتاج المعلومات
                                    الأساسية للإستمرار</h3>
                            ...
                            <div class="mt-4 flex flex-col gap-2">
                                <div class="w-full space-y-3">
                                    ...
                                    <select v-model="gender"
                                        class="form-control form-select h-[50px] appearance-none" type="select"
                                        name="select" id="gender" required>
                                ...
                                <div class="w-full space-y-3 dream_time">
                                    ...
                                    <DatePicker placeholder="mm/dd/yyyy" :max-date="new Date()"
                                        prevent-min-max-navigation v-model="age"
                                        model-type="yyyy-MM-dd" ref="datePicker" id="date" format="yyyy-MM-dd"
                                        select-text="اختيار" cancel-text="الغاء" required/>

                                    <!-- <InputError :message="errors.dream_time" /> -->
                                    <InputError :message="errors.age"  />
                                </div>
                                <div class="w-full space-y-3">
                                    <label class="block text-sm font-semibold xs:text-base" for="select">الحالة
                                        الإجتماعة</label>
                                    <select class="form-control form-select h-[50px] appearance-none" type="select"
                                        v-model="marital_status" name="select" id="select" required>
                                ...
                                <div class="w-full space-y-3">
                                    ...
                                    <input v-model="profession"
                                        class="form-control h-[50px] appearance-none" type="text" name="text" id="text"
                                        placeholder="ادخل مهنتك" dir="rtl" required />
...
const { tempAccount } = storeToRefs(useAccountStore());
...
const { defineField, errors, validate } = useForm({
    validationSchema: toTypedSchema(
        yup.object({
            age: yup
                .string()
                .default(data.value.profile.birthday).required('هذا الحقل مطلوب'),

            gender: yup
                .string()
                .default('male')
                .default(data.value.profile.gender).required('هذا الحقل مطلوب'),

            marital_status: yup
                .string()
                .default(data.value.profile.marital_status).required('هذا الحقل مطلوب'),

            profession: yup
                .string()
                .default(data.value.profile.profession).required('هذا الحقل مطلوب')
        })
    )
});
...
async function submitUpdate() {
    ...
    tempAccount.value.profile.gender = gender.value;
    tempAccount.value.profile.birthdate = age.value;
    tempAccount.value.profile.maritalStatus = marital_status.value;

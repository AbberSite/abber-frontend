<template>
  <div class="flex items-end justify-between pt-8 sm:w-full sm:pt-0">
    <h1 class="text-lg font-medium xs:text-xl lg:text-2xl" id="order-details-heading">الطلب #{{ order?.id }}</h1>

    <!-- Options buttons on desktop devices -->
    <div class="hidden items-center space-x-3 rtl:space-x-reverse sm:flex">
      <DetailsCompleteButton v-if="isBuyer || isSeller" :order="order" :is-seller="isSeller" :is-buyer="isBuyer" @showIt="showConfirmComplete = true" />
      <DetailsOrderQualityBtn v-if="isStaff && !order.content.well_provided" />
      <slot></slot>

      <div class="relative">
        <button class="flex items-center justify-center rounded-md border bg-white px-4 py-3 shadow-sm hover:bg-gray-50" type="button" @click="showDropdown = true" aria-expanded="false" aria-haspopup="true">
  ...
  <DetailsMobileOptions v-if="showNavigation" @open-modal="showMobileModal = true">
    ...
    <DetailsOrderQualityBtn v-if="isStaff && !order.content.well_provided" class="h-[50px] w-full" />
  ...
  <DetailsConfirmComplete
    :show="showConfirmCompleteCancelOrder"
    :order="order"
    :is-seller="isSeller"
    :is-buyer="isBuyer"
    :cancel-order="true"
    @cancelOrder="
      showConfirmCompleteCancelOrder = false;
      cancelOrder();
    "
...
const isBuyerOrSeller = () => {
  ...
  isStaff.value = user.value.user_type == "إدارة";
};

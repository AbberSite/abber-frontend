<template>
  <div class="flex items-end justify-between pt-8 sm:w-full sm:pt-0">
    ...
    <div class="hidden items-center space-x-3 rtl:space-x-reverse sm:flex">
      <a v-if="order?.status == 'new' || order?.status == 'in_progress'"
        ...
        @click.prevent="AcceptOrder" href="/profile/update">
        ...
        <template v-else>
          ...
          <span class="mt-1.5">إستلام الطلب</span>
        </template>
      </a>
      <div class="relative">
        ...
        <transition enter-active-class="transition-all" leave-active-class="transition-all"
          ...
          <DetailsOptionsDropdown @show-review="showReviewModal = true" v-if="showDropdown"
            v-on-click-outside="() => (showDropdown = false)" :user="user" />
        </transition>
      </div>
    </div>
  </div>
...
async function AcceptOrder() {
    loading.value = true;
    await updateOrderStatus(order.value?.id, 'complete');

    useNotification({ content: 'تم تسليم الطلب', type: 'success' });
    loading.value = false;
}
</script>

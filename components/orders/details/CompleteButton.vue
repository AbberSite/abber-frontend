<template>
  <a v-if="order?.status == 'awaiting_delivery' && order?.service" class="flex items-center justify-center space-x-1 rounded-md bg-gray-900 px-4 py-3 text-xs font-semibold text-white shadow-sm hover:bg-gray-800 rtl:space-x-reverse cursor-pointer" @click.prevent="emit('showIt')">
    ...
    <template v-else>
      ...
      <span class="mt-1.5" v-text="isSeller ? (order?.status == 'in_progress' ? 'تسليم الطلب' : 'إعادة فتح الطلب') : isBuyer ? 'إستلام الطلب' : ''"></span>
    </template>
  </a>
</template>

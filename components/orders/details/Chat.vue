<template>
    <div class="space-y-6 rounded-lg border border-gray-100 px-6 py-6 lg:col-span-2">
        <template v-for="(messages, index) in segmentedMessages">
            ...
            <div class="flex w-full" :dir="data.username == message.user.username ? 'rtl' : 'ltr' " v-for="message in messages">
                ...
                <div class="ms-3 w-full space-y-3">
                    <div class="w-fit min-w-[4rem] max-w-full space-y-2">
                        <div class="rounded-lg rounded-tr-none  px-4 py-4 text-right text-sm leading-loose"
                    ...
                    <div class="rounded-lg rounded-tr-none bg-sky-100 px-4 py-4 text-right text-sm leading-loose">
                        <div class="space-y-2">
                            <div class="space-x-2 rtl:space-x-reverse">
                                <span class="font-semibold">العنوان:</span><span>تفسير رؤية العقرب</span>
                            </div>
                            <div class="space-x-2 rtl:space-x-reverse">
                                <span class="font-semibold">الحلم:</span
                                ><span
                                    >هناك حقيقة مثبتة منذ زمن طويل وهي أن المحتوى المقروء لصفحة ما سيلهي القارئ عن
                                    التركيز على الشكل الخارجي للنص</span
                                >
                            </div>
                            <div class="space-x-2 rtl:space-x-reverse">
                                <span class="font-semibold">تأريخ الحلم:</span><span>11-02-2024</span>
                            </div>
                            <div class="space-x-2 rtl:space-x-reverse">
                                <span class="font-semibold">العمر:</span><span>29</span>
                            </div>
                            <div class="space-x-2 rtl:space-x-reverse">
                                <span class="font-semibold">الجنس:</span><span>ذكر</span>
                            </div>
                            <div class="space-x-2 rtl:space-x-reverse">
                                <span class="font-semibold">الحالة الإجتماعية:</span><span>أعزب</span>
                            </div>
                            <div class="space-x-2 rtl:space-x-reverse">
                                <span class="font-semibold">المهنة:</span><span>معلم</span>
                            </div>
                        </div>
                        <div class="flex justify-end">
                            <svg
        ...
        <div class="relative w-full">
            ...
            <div class="relative flex justify-center">
                <span class="bg-white px-4 pt-1 text-[13px]">13 شباط 2024</span>
            </div>
        </div>
        Expressor messages
        ...
        <div class="relative w-full">
            <textarea
                ...
                rows="5"
                placeholder="إدخل نص الرسالة هنا..."
                required></textarea>
            <!-- Toolbar -->
...
// const segmentedMessages = messages.value.reduce((acc, message) => {
...
function formatTime(_date: string) {
    ...
    const formattedTime = new Intl.DateTimeFormat('ar-AR', {
        hour: 'numeric',
        minute: '2-digit',
        hour12: true
    }).format(date);
...
const segmentedMessages = computed<{ [key: string]: Message[] }>(() => {
    const _messages: any = {};

<template>
  <div class="relative">
    <button
      class="relative flex items-center justify-center rounded-md border bg-white px-4 py-3 shadow-sm hover:bg-gray-50"
    ...
    <transition enter-active-class="transition-all" leave-active-class="transition-all"
      ...
      <div v-if="showOrdersDropdown && orders.length > 0"
        ...
        <template v-for="(order, index) in orders">
          <button v-on-click-outside="() => (showOrdersDropdown = false)" v-if="order.status === 'in_progress'"
            class="block w-full px-4 pb-1.5 pt-3 text-right text-sm font-medium hover:bg-gray-50" type="button"
            role="menuitem" tabindex="-1">تسليم طلب {{ order.buyer.first_name }}</button>
        </template>
      </div>
    </transition>
  </div>

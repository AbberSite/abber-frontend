<template>
  <div class="is-scroll w-full overflow-x-auto pt-6">
    ...
    <table v-else class="w-full text-sm ltr:text-left rtl:text-right">
      <slot>
        <DashTablesThead :headItems="actions ? { ...headItems, actions: 'إجراءات' } : headItems">
        </DashTablesThead>
        <tbody class="divide-y">
          <tr v-for="(row, rowIndex) in bodyItems" :key="rowIndex">
            <DashTablesRow v-for="(value, key, colIndex) in headItems" :key="colIndex"
              :content="getNestedValue(row, key)" :name="key" />
            <DashTablesRow v-if="actions">
              <DashTablesCellsActions :actions="actions" :data="row" />
            </DashTablesRow>
          </tr>
        </tbody>
      </slot>

<template>
    <th class="whitespace-nowrap pb-1 pe-4 pt-1 font-semibold" scope="col" @click="toggle()">
        <button
            class="flex w-full items-center justify-between space-x-3 rounded-md bg-gray-100 px-4 pb-2 pt-3 rtl:space-x-reverse "
            type="button" v-if="active">
            <span>{{ content }}</span>
            <svg class="flex-shrink-0" :class="[desc && 'scale-y-[-1]']" xmlns="http://www.w3.org/2000/svg" fill="none"
...
const { ticket, order, content } = toRefs(props);
const { filters } = storeToRefs(useTicketsStore());
const active = computed(() => {
    if (ticket.value == filters.value.ordering || filters.value.ordering == '-' + ticket?.value) {
        return true
    };
    return false;
});

const desc = computed(() => filters.value.ordering == '-' + ticket?.value ? true : false);
function toggle() {

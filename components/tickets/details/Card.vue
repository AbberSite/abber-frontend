<template>
  <div :class="boxStyle">
    ...
    <div :class="rowStyle">
      ...
      <div class="font-semibold" v-text="ticket.title"></div>
    </div>
    <div :class="rowStyle" v-if="ticket?.status == 'مفتوحة'">
      <div :class="rowTitleStyle">إغلاق التذكرة</div>
      <div class="font-semibold cursor-pointer" @click="closeTicket">
...
const rowTitleStyle = computed(() => (isMobile.value ? "text-sm font-semibold text-gray-500 xs:text-base" : "font-semibold text-gray-500"));

const closeTicket = computed(async () => {

...
<template>
  <div class="w-full pt-8">
    <div class="flex items-center justify-between">
      <form class="w-full sm:max-w-sm" method="GET">
        <div class="w-full space-y-3">
          <div class="relative">
            ...
            ></span>
            <input class="form-control h-[50px] px-12" type="search" name="q" id="search" placeholder="إبحث عن كشف حساب معين" required />
      ...
      <div class="relative">
        <button class="hidden h-[50px] items-center justify-center rounded-md border bg-white px-4 py-3 text-xs font-semibold shadow-sm hover:bg-gray-50 sm:flex" type="button" @click="pdfDropdown = true" aria-expanded="false" aria-haspopup="true">
        ...
        <transition enter-active-class="transition-all" leave-active-class="transition-all" enter-from-class="translate-y-4 opacity-0" leave-to-class="translate-y-4 opacity-0">
          <WalletStatementsPDFExport v-if="pdfDropdown" @close="pdfDropdown = false" v-on-click-outside="() => (pdfDropdown = false)" />
        </transition>
      </div>
    </div>

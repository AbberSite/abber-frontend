  ...
  <div class="w-full pt-8">
    <div class="flex items-center justify-between">
      <form class="w-full sm:max-w-sm" method="GET">
        <div class="w-full space-y-3">
          <div class="relative">
            ...
            ></span>
            <input class="form-control h-[50px] px-12" type="search" name="q" v-model="filters.search" id="search" placeholder="إبحث عن عملية مالية معينة" required />

            <button class="absolute h-[50px] items-center justify-center px-4 py-4 ltr:right-0 rtl:left-0 sm:hidden" type="button" @click="showModal = true">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" height="20" width="20">
      ...
      <div class="relative flex items-center gap-4">
        ...
        <button class="hidden h-[50px] items-center justify-center rounded-md border bg-white px-4 py-3 text-xs font-semibold shadow-sm hover:bg-gray-50 sm:flex" type="button" @click="filtersDropdown = true" aria-expanded="false" aria-haspopup="true">
        ...
        <KeepAlive>
          <transition enter-active-class="transition-all" leave-active-class="transition-all" enter-from-class="translate-y-4 opacity-0" leave-to-class="translate-y-4 opacity-0">
            <WalletTransactionsFiltersDropdown v-show="filtersDropdown" v-on-click-outside="() => (filtersDropdown = false)" />
          </transition>
        </KeepAlive>
      </div>
    </div>
    <div class="is-scroll w-full overflow-x-auto pt-6">
      <SkeletonsTable v-if="loading && transactions.length != 0" />

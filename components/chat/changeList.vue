  ...
  >
    <ul class="py-2 divide-light divide-y">
      <li @click="copyMessage()">
        <button
          type="button"
          class="flex hover:bg-gray-800 hover:text-gray-100 items-center px-5 py-3 w-full"
          title="نسخ الرسالة"
        >
          <svg
      ...
      <li
        v-if="message?.user.username == user?.username"
        @click="deleteMessage()"
      >
        <button
          type="button"
...
async function copyMessage() {
  try {
    ...
    alert(
      "Failed to copy message to clipboard. Please try again or use a different method."
    );
  }

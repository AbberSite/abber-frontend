<template>
    ...
    <main class="min-h-screen outline-none">
        ...
        <section class="relative flex min-h-screen w-full flex-col items-center justify-center px-4 pb-14 xs:px-6"
            ...
            <h1 class="sr-only" id="forget-password-heading">تعيين كلمة مرور جديدة</h1>
                ...
                <form method="POST" @submit.prevent="submit">
                    <fieldset class="space-y-7">
                        <div class="w-full space-y-3">
                            <label class="text-sm font-semibold xs:text-base" for="password">كلمة المرور الجديدة</label>
                            <div class="relative">
                                <input class="form-control h-[50px] appearance-none" :type="show ? 'text' : 'password'"
                                    v-model="password" name="password" id="password" placeholder="كلمة المرور الجديدة"
                                    required />
                                <button
                        ...
                        <div class="w-full space-y-3">
                            <label class="text-sm font-semibold xs:text-base" for="password">كلمة المرور الجديدة (مرة
                                أخرى)</label>
                            <div class="relative" x-data="{ show: false }">
                                <input class="form-control h-[50px] appearance-none" :type="show ? 'text' : 'password'"
                                    v-model="passwordConfirmation" name="password" id="password"
                                    placeholder="كلمة المرور الجديدة (مرة أخرى)" required />
                                <button
                                    class="absolute h-[50px] items-center justify-center px-4 py-4 text-gray-600 hover:text-gray-900 ltr:right-0 rtl:left-0"
                                    type="button" @click="show = !show" title="اظهار/اخفاء كلمة المرور">
                                    <svg :class="{ hidden: show }" xmlns="http://www.w3.org/2000/svg" fill="none"
                        ...
                        <div>
                            <!-- <a class="flex h-[50px] items-center justify-center rounded-md border border-transparent bg-gray-900 px-8 py-3 text-sm font-semibold text-white hover:bg-gray-800"
                                href="/accounts/password/reset/key/done"> <span class="mt-1.5">تعيين كلمة المرور</span></a> -->
    ...
    validationSchema: toTypedSchema(
        yup.object({
            ...
            passwordConfirmation: yup.string()
                .oneOf([yup.ref('password')], 'هذا الحقل يجب أن يطابق كلمة السر')
        }),
    ),
});
...
async function submit() {
    ...
    await useFetch("/api/auth/new-password", {
        ...
        onResponse({ request, response, options }) {
            ...
            if (response.status === 200) {
                useRouter().push({ name: 'accounts-new-password-done' })
            }

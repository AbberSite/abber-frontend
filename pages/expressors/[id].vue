            ...
            <template v-else>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="hidden items-center space-x-3 rtl:space-x-reverse sm:flex" v-if="!['إدارة', 'معبر'].includes(user?.user_type)">
                        <NuxtLink
                            class="flex items-center justify-center space-x-1 rounded-md bg-gray-900 px-4 py-3 text-xs font-medium text-white shadow-sm hover:bg-gray-800 rtl:space-x-reverse"
                            :to="{ name: 'orders-make' }">
                ...
                <div class="flex flex-col items-center pt-8 sm:block">
                    <span class="text-lg font-medium">{{ `${expressor?.seller.first_name}
...
onMounted(async () => {
    const data = await useDirectApi(`/services/services/${route.params.id}`);
    const dataExpressor = await useDirectApi(`/expressors/expressors/${route.params.id}`);
    const ratesData = await useDirectApi(`/services/services/${route.params.id}/ratings/?ordering=-date`);

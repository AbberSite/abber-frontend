    ...
    <section class="relative flex min-h-screen w-full flex-col items-center px-4 pb-36 pt-20 xs:px-6 md:pt-32 lg:px-8 xl:pb-44" aria-labelledby="change-password-heading">
      ...
      <div class="mx-auto w-full max-w-sm pt-10">
        <form @submit.prevent="submit">
          <fieldset class="space-y-7">
            <div class="w-full space-y-3">
              <div class="relative">
                <OtpInput v-model="pin" v-if="pin.length != 4" />
                <OtpInput v-model="pinConfirmation" @done="allowConfirm = true" v-if="pin.length == 4" />
            ...
            <div>
              <PrimaryButton :loading="loading" class="w-full" :disabled="!(allowConfirm && pin.length == 4 && pinConfirmation.length == 4)" v-if="pin.length == 4">تأكيد</PrimaryButton>
            </div>
          </fieldset>
        </form>
        <div class="pt-8 text-center text-sm xs:text-base">
          ...
          <button
            v-else
            @click="pin = [];pinConfirmation = [];errors.pin = [];"

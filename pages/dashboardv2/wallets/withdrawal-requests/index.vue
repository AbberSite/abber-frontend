  ...
  <ClientOnly>
    <ModifyModal v-if="showModal" @close="showModal = false" title="الإجراءات">
        ...
        <div :class="$style.style_every_dev" v-if="dataSelection.status">
          <PrimaryButton @click.prevent="submit()"  :loading="loadingButton"
            >تأكيد</PrimaryButton
          >
        </div>
    </ModifyModal>
...
const { filters, list, loading, filtersCount, pagination } = storeToRefs(
  useDashboardWithdrawalRequestsStore()
);
...
provide(
  "otherCheckStatus",
  (data) =>
    (!data?.invoice && (data?.status === "1" || data?.status === "2")) ||
    (data?.invoice && data?.status === "1"),
  true
);
...
const submit = async () => {
  errors.invoice =
    !dataSelection.invoice && dataSelection.status == "2"
      ? "يجب ملء هذا الحقل بملف"
      : null;

  errors.refuse_reason =
    !dataSelection.refuse_reason && dataSelection.status == "3"
      ? "يجب ملء هذا الحقل"
      : null;
  if (errors.invoice || errors.refuse_reason) {
    return;
  }
  ...
  if (dataSelection.invoice) {
    const randomFileName = `invoice_${
      dataSelection.id
    }_${new Date().toISOString()}.${dataSelection.invoice.name
      .split(".")
      .pop()}`;
    const blob = dataSelection.invoice.slice(
      0,
      dataSelection.invoice.size,
      dataSelection.invoice.type
    );
    const renamedInvoice = new File([blob], randomFileName, {
      type: dataSelection.invoice.type,
    });
  ...
  try {
    ...
    useNotification({
      type: "danger",
      content: "لقد حدث خطأ ما، يرجى إعادة المحاولة.",
    });
  } finally {

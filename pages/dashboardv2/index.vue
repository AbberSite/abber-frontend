<template>
  ...
  <div class="grid w-full gap-x-4 md:grid-cols-2 lg:gap-x-6">
    <DashboardAnalyticsEarings
      total="150000"
      recieved="140000000"
      pending="14200"
      class="my-2 sm:my-0"
  ...
  >
    <!-- <div class="col-span-12 inline-flex items-center lg:col-span-8 overflow-x-auto py-6 px-6 sm:col-span-6 sm:pt-0 space-x-3 space-x-reverse is-scroll w-full" >
      <DashboardAnalyticsCard v-for="(card, index) in cards" :key="index" :title="card.title" :count="card.count" :subtitle="card.subtitle" :iconBackgroundClass="`${card.iconBackgroundClass} rounded-[14px]`">
         <template #icon><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" height="20" width="20"  v-html="card.icon"></svg></template>
         <template #background-icon>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" height="56" width="56" class="-translate-x-1/4 translate-y-1/4" v-html="card.icon"></svg>
         </template>
...
const earingsData = reactive([
  { title: "الأرباح المستحقة", amount: 1450 },
  { title: "الأرباح المرسلة", amount: 120 },
  { title: "الأرباح المعلقة", amount: 222, down: true },
]);
const ordersInfo = reactive([
  { title: "التحويلات الماليه", amount: 120 },
  { title: "رسوم البطاقة", amount: 80 },
  { title: "رسوم الموقع", amount: 80 },
  { title: "القيمة المضافة", amount: 90 },
]);
const datasets = reactive([
  ...
  {
    label: "الطلبات المتأخرة بالتسليم",
    data: [3, 6, 2, 8, 1, 4, 1],
    borderWidth: 1,
    fill: false,
  ...
  {
    label: "الطلبات المتأخرة بالإستلام",
    data: [3, 6, 2, 8, 1, 4, 1],
    borderWidth: 1,
    fill: false,

              ...
              <button v-for="(ticket, index) of tickets" :key="index" class="flex w-full items-center justify-between px-6 py-4 hover:bg-gray-50" :class="{ 'bg-gray-100': ticketId == ticket.id }" type="button" @click="setCurrentTicket(ticket)">
                <span class="flex items-center">
                  ...
                  <span class="ms-3 flex flex-col pt-1 text-right">
                    <span class="text-sm font-medium" v-text="ticket.user?.first_name" ></span>
                    <span class="pt-1 text-xs font-medium text-gray-600" v-text="ticket.title" ></span>
                  </span>
                </span>
                <span v-if="ticket.status === 'مفتوحة'" class="rounded-full bg-gray-900 px-4 pb-1 pt-1.5 text-xs font-medium text-white">1</span>
              </button>
            </div>
